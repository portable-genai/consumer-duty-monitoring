...
from consumer_duty_monitoring.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    planted = replace_citation_snippet(
        CANONICAL_RESULT, f"contact NRIC {sample_cases.PLANTED_NRIC} on file"
    )
    payload = repr(
        result_to_review(planted, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT).to_payload()
    )
    assert sample_cases.PLANTED_NRIC not in payload
    assert "REDACTED" in payload


def replace_citation_snippet(assessment: OutcomeAssessment, snippet: str) -> OutcomeAssessment:
    """A copy whose first citation carries ``snippet`` (a planted id, for the redaction proof)."""
    first = assessment.citations[0]
    planted = Citation(source_id=first.source_id, title=first.title, snippet=snippet)
    return replace(assessment, citations=(planted, *assessment.citations[1:]))
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    assessment = build_service(local).assess(
        sample_cases.TENANT, policy_for(local), actor=sample_cases.ACTOR, as_of=sample_cases.AS_OF
    )
    assert assessment.requires_human_review is True
    assert assessment.citations, "an offline answer must still be cited"
    assert assessment.review_ref, "an escalated assessment must be routed (rule R8)"

    onprem = build_container(local_settings(profile="onprem"))
    with pytest.raises(NotImplementedError):
        build_service(onprem).assess(
            sample_cases.TENANT,
            policy_for(onprem),
