...
from consumer_duty_monitoring.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from consumer_duty_monitoring.ports.identity import (
    VERIFIED,
...
_API_MODULE = "consumer_duty_monitoring.api.app"
...
#: Every port is listed because the loader refuses a partial ``adapters:`` block: the block must
#: bind exactly the declared ports, in every profile, or ``Settings.load`` raises and the guard's
#: identity read fails closed to CLIENT_ASSERTED, which is not what these tests are about.
_PKG = "consumer_duty_monitoring"

#: (port, local class, onprem class). The module file is named for the port. The managed (gcp)
#: entry is the LOCAL adapter for every DATA port, so these tests carry no cloud SDK; IDENTITY is
#: the exception, keeping its real gcp binding because it is the adapter under test.
_REBOUND_PORTS: tuple[tuple[str, str, str], ...] = (
    ...
    ("review_router", "LocalReviewRouter", "OnPremReviewRouter"),
    ("signal_source", "LocalSignalSource", "OnPremSignalSource"),
    ("warehouse", "LocalWarehouseExport", "OnPremWarehouseExport"),
)


def _rebound_adapter_lines() -> list[str]:
    lines = ["adapters:"]
    for port, local_cls, onprem_cls in _REBOUND_PORTS:
        if port == "identity":
            gcp = f"{_PKG}.adapters.gcp.identity:IapIdentityAdapter"
        else:
            gcp = f"{_PKG}.adapters.local.{port}:{local_cls}"
        lines += [
            f"  {port}:",
            f"    local: {_PKG}.adapters.local.{port}:{local_cls}",
            f"    gcp: {gcp}",
            f"    onprem: {_PKG}.adapters.onprem.{port}:{onprem_cls}",
        ]
    return lines
...
def test_a_verified_assertion_is_served_so_the_refusals_above_mean_something(
    ...
    The escalation is ROUTED in the same request (rule R8), driven by the VERIFIED principal's
    tenant and never by anything in the body: the assessment runs over the tenant the assertion
    names (``hd``), so the seeded breaching tenant is the hosted domain on THIS claim set, and the
    request body is ignored. That the result escalates and carries a routing reference is what
    proves a verified assertion reaches the real pipeline rather than a stub.
    """
    install_fake_google(monkeypatch, {**GOOD_CLAIMS, "hd": "demo-bank"})
    module = _managed_app(monkeypatch, tmp_path, AUDIENCE)
    with TestClient(module.app, client=(LAN_PEER, 50000)) as client:
        resp = client.post(
            "/v1/assess",
            json={"tenant": "ignored-by-the-server"},
            headers={_IAP_ASSERTION_HEADER: signed_assertion()},
        )
    assert resp.status_code == 200
    body = resp.json()
    assert body["tenant"] == "demo-bank", "the verified principal's tenant drives the assessment"
