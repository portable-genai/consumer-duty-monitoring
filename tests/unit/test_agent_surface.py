...
2. **A third place an escalation stops.** The API and the CLI both route an escalated assessment
   to human review. If the agent path only returned the flag, rule R8 would hold on two surfaces
...
from consumer_duty_monitoring.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    build_agent_card,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "consumer-duty-monitoring"
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    caller the evidence for its own tenant. A tool result goes into a model's context, so the
    citation snippet that carried a planted identifier is masked on the way out.
    """
    result = run_assessment(
        PII_TENANT,
        settings=local_settings(),
    ...
    assert PLANTED_NRIC not in rendered
