# ARCHITECTURE: Consumer Duty Monitoring (`consumer-duty-monitoring`)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`CONSUMERDUTY_PROFILE`) swaps the
...
## Layout (`src/consumer_duty_monitoring/`)
