...
Rewrites the python package name, the ``CONSUMERDUTY_`` env-var prefix, the baked-in cloud
resource stem and the distribution / git id across the tree, so a fork does not have to hunt
them down by hand. It is a mechanical text plus directory rename, deliberately conservative: it
prints a plan, writes nothing unless ``--yes``, and by default leaves docs prose alone (pass
...
* **The console script is named after the package.** ``pyproject.toml`` declares
  ``consumer_duty_monitoring = "consumer_duty_monitoring.cli.main:main"``, so there is no
  separate ``--cli`` knob: renaming the package renames the ``assess`` command too.
* **The env prefix has two spellings.** ``CONSUMERDUTY_`` prefixes every variable, and the bare
  token ``CONSUMERDUTY`` is carried as ``render_env_prefix`` in
...
outcome-test pack, region, IdP, the signal feeds, eval golden set) that this script does NOT do
for you.
"""
...
_OLD_PACKAGE = "consumer_duty_monitoring"
_OLD_ENV_PREFIX = "CONSUMERDUTY_"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (CONSUMERDUTY_PROFILE), never
    # mid-word.
...
def main() -> int:
    ...
    print(f"  {_OLD_DIST!r:32} -> {(args.dist or args.resource)!r}   (distribution / git id)")
    print(f"  {_OLD_RESOURCE!r:32} -> {args.resource!r}   (Terraform name_prefix)")
    print(f"  {_OLD_PACKAGE!r:32} -> {args.package!r}   (package and console script)")
    print(
    ...
    print("Then finish the human steps in docs/ADOPTING.md (outcome pack, region, IdP, eval).")
    return 0
