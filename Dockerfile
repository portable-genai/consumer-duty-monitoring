# Consumer Duty Monitoring (consumer-duty-monitoring) serving image.
#
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    CONSUMERDUTY_PROFILE=gcp \
    CONSUMERDUTY_SETTINGS=/app/config/settings.yaml \
    PORT=8080
...
CMD ["sh", "-c", "python -m consumer_duty_monitoring.managed_readiness && exec uvicorn consumer_duty_monitoring.api.app:app --host 0.0.0.0 --port ${PORT:-8080}"]
