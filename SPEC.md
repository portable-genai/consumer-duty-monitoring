# SPEC: Consumer Duty Monitoring (`consumer-duty-monitoring`)
...
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  with routing on and no console configured; `CONSUMERDUTY_REVIEW_ROUTING=off` is the stated way to
...
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `CONSUMERDUTY_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters
