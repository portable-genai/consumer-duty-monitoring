...
``CONSUMERDUTY_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CONSUMERDUTY_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from .schemas import AssessmentResponse, AssessRequest, HealthResponse
...
_TOKEN_ENV = "CONSUMERDUTY_S2S_TOKEN"
_INSECURE_DEMO_ENV = "CONSUMERDUTY_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CONSUMERDUTY_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CONSUMERDUTY_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CONSUMERDUTY_S2S_AUDIENCE"
_API_HOST_ENV = "CONSUMERDUTY_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CONSUMERDUTY_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CONSUMERDUTY_PROFILE`` must never be allowed to
    ...
    ``CONSUMERDUTY_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CONSUMERDUTY_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "consumer-duty-monitoring"
...
_APP_TITLE = "Consumer Duty Monitoring"
...
# `X-Search-Used` on the same response. The console's pills read those two headers, so what a pill
# names is what answered, never what configuration says would. The local narrator notes its stub
# name; the managed narrator is a placeholder that raises, and a draft that fails ships the
# deterministic narration, so such a request notes nothing and sends neither header.
...
#
# Note what is NOT in this expression: CONSUMERDUTY_S2S_TOKEN. A service
...
@app.post("/v1/assess", response_model=AssessmentResponse, tags=["artifacts"])
def assess(
    request: AssessRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> AssessmentResponse:
    """Run the Consumer Duty outcome assessment for the verified principal's tenant.
...
def read_assessment(
    ...
    """Fetch one stored assessment, enforcing the tenant boundary with a 403 (never a 404).

    The store hands the record over unfiltered and the DOMAIN refuses to serve it across tenants,
    so the boundary is enforced once, where every surface inherits it.
    """
    container = _container()
    service = build_service(container)
    try:
        assessment = service.read_assessment(assessment_id, principal_tenant=principal.tenant)
    except TenantAccessDeniedError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc
    except KeyError as exc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"no assessment {assessment_id}"
        ) from exc
