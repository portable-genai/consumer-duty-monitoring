...
class AgentCard:
    ...
    provider: str = "consumer-duty-monitoring"
...
SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="run_assessment",
        name="Consumer Duty assessment",
        description=(
            "Run the four deterministic Consumer Duty outcome tests over a tenant's signals and "
            "products, record an already-redacted audit event, and ROUTE the assessment to human "
            "review when any test breaches or is a gap (rule R8). Every number and verdict is "
            "computed by pure stdlib code, never by a model."
        ),
    ),
    AgentSkill(
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://consumer-duty-monitoring.{settings.region}.internal.example/a2a"
