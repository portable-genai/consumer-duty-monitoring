...
DEFAULT_ACTOR = "consumer-duty-monitoring-agent"
...
def run_assessment(
    ...
    """Run the Consumer Duty outcome assessment for a tenant and route it when it escalates.

    Runs the four deterministic outcome tests over the tenant's signals and products, writes an
    already-redacted audit event, and, when any test breaches or is a gap, submits the assessment
    to the human-review console (rule R8).

    Args:
      tenant: The tenant to assess.
...
TOOL_FUNCTIONS = (run_assessment, verify_audit_trail)
