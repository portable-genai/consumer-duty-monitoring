...
One env var (``CONSUMERDUTY_PROFILE``) selects the adapter family for every
...
_PROFILE_ENV = "CONSUMERDUTY_PROFILE"
_SETTINGS_ENV = "CONSUMERDUTY_SETTINGS"
...
#: Profiles that mean "running on managed cloud infrastructure", for the model pill's title.
...
#: model pill before any answer arrives). Named once here so rebinding it for a profile changes
#: the pill in the same edit.
...
class ProfileChoice:
    ...
    The variable is ``CONSUMERDUTY_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CONSUMERDUTY_PROFILE", "local")``: that fallback reads an
...
_PKG = "consumer_duty_monitoring"

DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    "assessment_store": {
        "local": f"{_PKG}.adapters.local.assessment_store:LocalAssessmentStore",
        "gcp": f"{_PKG}.adapters.gcp.assessment_store:FirestoreAssessmentStore",
        "onprem": f"{_PKG}.adapters.onprem.assessment_store:OnPremAssessmentStore",
    },
    "audit": {
    ...
    "consent": {
        "local": f"{_PKG}.adapters.local.consent:LocalConsentLookup",
        "gcp": f"{_PKG}.adapters.gcp.consent:ConsentStoreLookup",
        "onprem": f"{_PKG}.adapters.onprem.consent:OnPremConsentLookup",
    },
    "identity": {
    ...
    "narration": {
        ...
        "gcp": f"{_PKG}.adapters.gcp.narration:GeminiNarrator",
        "onprem": f"{_PKG}.adapters.onprem.narration:OnPremNarrator",
    },
    "product_governance": {
        "local": f"{_PKG}.adapters.local.product_governance:LocalProductGovernance",
        "gcp": f"{_PKG}.adapters.gcp.product_governance:DocumentAiProductGovernance",
        "onprem": f"{_PKG}.adapters.onprem.product_governance:OnPremProductGovernance",
    },
    "review_router": {
    ...
    "signal_source": {
        "local": f"{_PKG}.adapters.local.signal_source:LocalSignalSource",
        "gcp": f"{_PKG}.adapters.gcp.signal_source:BigQuerySignalSource",
        "onprem": f"{_PKG}.adapters.onprem.signal_source:OnPremSignalSource",
    },
    "warehouse": {
        "local": f"{_PKG}.adapters.local.warehouse:LocalWarehouseExport",
        "gcp": f"{_PKG}.adapters.gcp.warehouse:BigQueryWarehouseExport",
        "onprem": f"{_PKG}.adapters.onprem.warehouse:OnPremWarehouseExport",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CONSUMERDUTY_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "CONSUMERDUTY_REVIEW_ROUTING"
...
class Settings:
    ...
    project_id: str = ""
    #: The outcome-test pack this deployment scores against. Empty selects the shipped reference
    #: pack. It is a PATH rather than tuned constants because the harm, drift, price-vs-value and
    #: vulnerable-customer thresholds are the firm's Consumer Duty risk appetite (practice B4).
    outcome_pack_path: str = ""
    #: Offline assessment store (SQLite). ``:memory:`` is the ephemeral default the gate uses.
    assessment_path: str = ":memory:"
    #: Where the offline warehouse export appends JSON Lines. Empty means in-memory only, which is
    #: right for the gate and wrong for anything durable.
    warehouse_path: str = ""
    #: The managed warehouse target, ``project.dataset.table``. Empty makes the managed export
    #: refuse rather than invent a destination.
    warehouse_table: str = ""
    #: Base URL of the consent and preference store the managed consent lookup reads (read-only).
    #: Empty makes the managed lookup refuse rather than read an unknown preference as consent.
    consent_url: str = ""
    #: The managed narration model. It produces no number and no verdict; see ports/narration.py.
    narration_model: str = "gemini-3.5-flash"
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            outcome_pack_path=str(data.get("outcome_pack_path") or ""),
            assessment_path=str(data.get("assessment_path") or ":memory:"),
            warehouse_path=str(data.get("warehouse_path") or ""),
            warehouse_table=str(data.get("warehouse_table") or ""),
            consent_url=str(data.get("consent_url") or ""),
            narration_model=str(data.get("narration_model") or "gemini-3.5-flash"),
...
class Container:
    ...
    @cached_property
    def signal_source(self) -> SignalSourcePort:
        adapter = self._bind("signal_source")
        assert isinstance(adapter, SignalSourcePort)
        return adapter

    @cached_property
    def product_governance(self) -> ProductGovernancePort:
        adapter = self._bind("product_governance")
        assert isinstance(adapter, ProductGovernancePort)
        return adapter

    @cached_property
    def consent(self) -> ConsentLookupPort:
        adapter = self._bind("consent")
        assert isinstance(adapter, ConsentLookupPort)
        return adapter
    ...
    @cached_property
    def assessment_store(self) -> AssessmentStorePort:
        adapter = self._bind("assessment_store")
        assert isinstance(adapter, AssessmentStorePort)
        return adapter

    @cached_property
    def warehouse(self) -> WarehouseExportPort:
        adapter = self._bind("warehouse")
        assert isinstance(adapter, WarehouseExportPort)
        return adapter
...
def warn_switched_off(switched_off: tuple[str, ...]) -> None:
    ...
    The agent tools can build a container per tool call, so a warning in
